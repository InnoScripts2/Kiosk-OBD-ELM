...
├── publish-kiosk-frontend.ps1          ✅ Публикация dist/ в бакет Supabase
...
- `publish-kiosk-frontend.ps1` — собирает `platform/ui/web/kiosk-frontend`, при необходимости очищает
  бакет (`-ClearBucket`), выполняет `supabase storage cp -r dist ss:///kiosk-ui` и может синхронизировать
  строку `kiosk_url_remote` (`-UpdateAndroidString`, `-AppendCacheBuster`). Используется для поддержания
  резервной копии UI в Supabase Storage и актуального fallback в Android.
