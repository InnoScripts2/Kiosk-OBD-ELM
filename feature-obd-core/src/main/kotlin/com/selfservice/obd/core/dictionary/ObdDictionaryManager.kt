...
import com.selfservice.obd.core.dtc.BrandOverrideDtcProvider
...
internal constructor(
    ...
    manufacturerProvider: ManufacturerDtcProvider? = null
) {
    private val brandOverrideProvider = BrandOverrideDtcProvider()
    init {
    ...
    @Volatile private var manufacturerDtcProvider: ManufacturerDtcProvider? = manufacturerProvider
    ...
    fun lookupDtcWithManufacturerFallback(code: String, manufacturer: String? = null): ObdDtcDefinition? {
        val normalizedManufacturer = manufacturer?.takeIf { it.isNotBlank() }
        if (normalizedManufacturer != null) {
            manufacturerDtcProvider?.find(normalizedManufacturer, code)?.let { return it }
            brandOverrideProvider.find(normalizedManufacturer, code)?.let { return it }
        }
        return lookupDtc(code)
    }

    fun lookupManufacturerDtc(manufacturer: String, code: String): ObdDtcDefinition? {
        return manufacturerDtcProvider?.find(manufacturer, code)
                ?: brandOverrideProvider.find(manufacturer, code)
    }
    ...
    fun manufacturerCatalog(manufacturer: String): List<ObdDtcDefinition> {
        val primary = manufacturerDtcProvider?.list(manufacturer).orEmpty()
        if (primary.isNotEmpty()) {
            return primary
        }
        return brandOverrideProvider.list(manufacturer)
    }

    fun manufacturerCount(): Int {
        val providerSize = manufacturerDtcProvider?.size() ?: 0
        return if (providerSize > 0) providerSize else brandOverrideProvider.size()
    }

    fun manufacturerList(): List<String> {
        val primary = manufacturerDtcProvider?.manufacturers().orEmpty()
        return if (primary.isNotEmpty()) primary else brandOverrideProvider.manufacturers()
    }

    fun attachManufacturerProvider(provider: ManufacturerDtcProvider) {
        manufacturerDtcProvider = provider
    }

    fun detachManufacturerProvider() {
        manufacturerDtcProvider = null
    }
