...
class PassThruSessionManager(
        ...
        private val dictionarySyncCoordinator: DictionarySyncCoordinator? = null
) {
    ...
    suspend fun startWithAdapter(
        ...
        dictionarySyncCoordinator?.triggerSync(force = false)
    ...
    suspend fun syncDictionaries(force: Boolean = false): DictionarySyncResult? {
        val coordinator = dictionarySyncCoordinator ?: return null
        return coordinator.triggerSync(force)
    }

    fun lastDictionarySync(): DictionarySyncResult? = dictionarySyncCoordinator?.lastResult()
    ...
    fun createConfig(
        ...
        configFactory.create(initialAdapter)
        }
