...
/**
 * Привязки таблицы Supabase для очереди доставки отчётов толщиномера.
 */
object ThicknessReportDeliverySupabaseSchema {

    const val TABLE_NAME: String = "thickness_report_deliveries"
    ...
    fun toRows(
        ...
        val reportId = ThicknessReportSupabaseSchema.stableId(record)
    ...
    private fun extractDeliveries(
        ...
        val customer = when (customerRaw) {
            is Map<*, *> -> customerRaw
            is JSONObject -> customerRaw.toMapCompat()
        ...
        if (emailRaw.isNotEmpty()) {
            ...
            if (isValidEmail(normalizedEmail)) {
                ...
                Log.w(TAG, "Skipping thickness report email delivery: invalid address")
            }
        }
        ...
        if (phone.isNotEmpty()) {
            ...
            if (normalized.isNotEmpty()) {
                ...
                Log.w(TAG, "Skipping thickness report SMS delivery: invalid phone")
            }
        }
    ...
    private fun buildDeliveryMetadata(
        ...
        val metadata = record.metadata
        val deliveryMetadata = LinkedHashMap<String, Any?>(4)
        metadata["vehicle"]?.let { deliveryMetadata["vehicle"] = it }
        metadata["vehicle_type"]?.let { deliveryMetadata["vehicle_type"] = it }
        metadata["stats"]?.let { deliveryMetadata["stats"] = it }
        metadata["analysis"]?.let { deliveryMetadata["analysis"] = it }
        metadata["summary"]?.let { deliveryMetadata["summary"] = it }
        metadata["customer"]?.let { deliveryMetadata["customer"] = it }
    ...
    private const val TAG = "ThicknessReportDelivery"
