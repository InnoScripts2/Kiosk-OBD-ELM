...
class DiagnosticsJavascriptBridge @VisibleForTesting internal constructor(
    ...
    private fun String.toJson(manufacturer: String?): JSONObject {
        ...
        return JSONObject().apply {
            ...
            if (definition != null) {
                ...
                definition.status.value.let { put("status", it) }
                definition.sources.takeIf { it.isNotEmpty() }?.let { putArray("sources", it) }
                definition.brandOverrides.takeIf { it.isNotEmpty() }?.let { putArray("brandOverrides", it) }
                definition.details?.let { put("details", it.toJson()) }
                put("severity", inferDtcSeverity(this@toJson, definition))
            } else {
                put("found", false)
                put("severity", inferDtcSeverity(this@toJson, null))
            }
            description?.let { put("description", it) }
            put("descriptionSource", descriptionSource(definition))
            put("descriptionMissing", description == null)
    ...
    private fun ObdDtcEntry.toJson(): JSONObject = JSONObject().apply {
        put("code", code)
        val def = definition
        val description = preferredDescription(def)
        def?.let {
            put("label", it.label)
            put("system", it.system.value)
            it.notes?.let { notes -> put("notes", notes) }
            it.details?.let { details -> put("details", details.toJson()) }
            it.sources.takeIf { sources -> sources.isNotEmpty() }?.let { sources -> putArray("sources", sources) }
            it.brandOverrides.takeIf { overrides -> overrides.isNotEmpty() }?.let { overrides -> putArray("brandOverrides", overrides) }
            put("status", it.status.value)
        }
        description?.let { put("description", it) }
        put("descriptionSource", descriptionSource(def))
        put("descriptionMissing", description == null)
        put("severity", inferDtcSeverity(code, def))
    }

    private fun JSONObject.putArray(key: String, values: Collection<String>) {
        put(key, JSONArray().apply {
            values.forEach { value -> put(value) }
        })
    }

    private fun ObdDtcDefinition.Details.toJson(): JSONObject = JSONObject().apply {
        preferred?.let { put("preferred", it) }
