 ...
 */
export async function createIntent(amount, service, sessionId, currency = 'RUB', meta = {}) {
  ...
  return /** @type {Promise<PaymentIntent>} */ (response.json());
}
...
export async function getStatus(intentId) {
  ...
  return /** @type {Promise<PaymentStatus>} */ (response.json());
}
...
export async function cancelIntent(intentId) {
  ...
  return /** @type {Promise<{ok: boolean, status: string}>} */ (response.json());
}
 ...
 * @property {(status: string, intent: PaymentStatus) => void=} onStatusChange
 */
 ...
 * 
 * @param {string} intentId - ID платежного намерения
 * @param {PollingOptions} [options] - Опции polling
 * @returns {Promise<{confirmed: boolean, status: string, intent?: PaymentStatus}>}
 */
export async function pollStatus(intentId, options = {}) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      if (error instanceof Error && error.message === 'intent_not_found') {
