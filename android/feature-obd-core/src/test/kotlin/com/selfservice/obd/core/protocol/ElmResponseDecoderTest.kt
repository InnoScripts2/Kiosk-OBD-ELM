...
class ElmResponseDecoderTest {
    ...
    @Test
    fun `parsePid decodes catalyst temperature payload`() {
        val definition = requireNotNull(PidCatalog.find("0x01", "0x3C"))
        val sample = ElmResponseDecoder.parsePid(definition, "41 3C 1A F8", timestampMillis = 2_500L)
        assertNotNull(sample)
        assertEquals("1AF8", sample.rawHex)
        assertEquals(listOf(0x1A.toByte(), 0xF8.toByte()), sample.rawPayload.asList())
        assertEquals("degC", sample.unit)
        assertEquals(650.4, sample.value ?: error("value expected"), 0.01)
    }

    @Test
    fun `parsePid decodes engine oil temperature payload`() {
        val definition = requireNotNull(PidCatalog.find("0x01", "0x5C"))
        val sample = ElmResponseDecoder.parsePid(definition, "41 5C 7B", timestampMillis = 2_600L)
        assertNotNull(sample)
        assertEquals("7B", sample.rawHex)
        assertEquals(listOf(0x7B.toByte()), sample.rawPayload.asList())
        assertEquals("degC", sample.unit)
        assertEquals(83.0, sample.value ?: error("value expected"), 0.01)
    }
    ...
    @Test
    fun `parseDtc falls back to manufacturer catalog`() {
        val manager = ObdDictionaryManager.shared
        val provider = ManufacturerDtcProvider(
                DtcDataLoader.loadDatabase(
                        """
                                {
                                    "Toyota": [
                                        {"code": "P3900", "description": "Custom manufacturer-only code", "source": "unit-test"}
                                    ]
                                }
                        """.trimIndent()
                )
        ...
        manager.attachManufacturerProvider(provider)
        try {
            val batch =
                    ElmResponseDecoder.parseDtc(
                                "43 39 00 00 00 00",
            ...
            val entry = batch.entries.single()
                        assertEquals("P3900", entry.code)
            val definition = entry.definition ?: error("manufacturer definition expected")
                        assertEquals("Custom manufacturer-only code", definition.label)
        } finally {
