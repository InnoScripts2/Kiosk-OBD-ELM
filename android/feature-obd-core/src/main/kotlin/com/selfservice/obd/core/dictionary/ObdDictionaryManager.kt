...
internal constructor(
    ...
    fun lookupDtcWithManufacturerFallback(code: String, manufacturer: String? = null): ObdDtcDefinition? {
        val primary = lookupDtc(code)
        if (primary != null || manufacturer.isNullOrBlank()) {
            return primary
        }
    ...
    fun manufacturerCatalog(manufacturer: String): List<ObdDtcDefinition> {
        return manufacturerDtcProvider?.list(manufacturer).orEmpty()
    }

    fun manufacturerCount(): Int = manufacturerDtcProvider?.size() ?: 0

    fun manufacturerList(): List<String> = manufacturerDtcProvider?.manufacturers().orEmpty()
