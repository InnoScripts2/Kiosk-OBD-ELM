...
The kiosk UI is published from Supabase Storage (`kiosk-ui` bucket on project `ddaunoxyguqiejrjtwsf`).

1. Copy `.env.example` to `.env.local` (this file stays untracked) and fill in:
  - `VITE_SUPABASE_URL=https://ddaunoxyguqiejrjtwsf.supabase.co`
  - `VITE_SUPABASE_ANON_KEY=<anon key from Supabase dashboard>`
  The values live in Vault and are mirrored in `09-docs/02-application/security/local-credential-notes.local.md`.
2. Install the Supabase CLI (`npm install -g supabase`) and authenticate via `supabase login`.
3. Link the local workspace to the project: `supabase link --project-ref ddaunoxyguqiejrjtwsf`.
4. Build the frontend: `npm run build`.
5. Upload the bundle to Storage (see `09-docs/02-application/plans/supabase-setup.md` for the complete checklist):
  ```bash
  ...
  ```
6. Validate deployment: `supabase storage list kiosk-ui` and open `https://ddaunoxyguqiejrjtwsf.supabase.co/storage/v1/object/public/kiosk-ui/index.html`.
