...
## Следующие шаги (25.11.2025)

1. **Толщиномер** — вынести сетку 60 точек и прогресс в модуль `src/screens/thk-measure.js`, получать состояние из REST `/api/thk/session` и обновлять через `thicknessStore`.
2. **Диагностика OBD** — разделить логику на четыре экрана (`obd-contact`, `obd-vehicle`, `obd-scan`, `obd-results`), подписать их на `device-status` и переиспользовать существующий WebSocket (`deviceStatus`), не дергая BLE напрямую.
3. **Платежи** — подключить `core/payment-client` к Supabase intents: после миграций держим конфиг в `.env.local`, dev-режим оставляем за флагом.
4. **Supabase интеграция** — после разделения миграций добавить клиент `src/api/supabase.js`, который будет читать таблицы `diagnostics_reports`/`thickness_reports` и отображать статусы на экране завершения.
5. **Модульность** — убрать оставшийся inline-скрипт из `index.html`, оставив только `<script type="module" src="/src/main.js"></script>` и привязку глобальных data-атрибутов для CSS.

> Эти шаги синхронизированы с задачами сессий 17–23 из `plan-80-session-roadmap.md` и не конфликтуют с уже реализованными Kotlin/Node сервисами.
```
